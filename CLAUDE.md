...
Credentials and connection details are supplied via a git-ignored `.env` file at the repo root (`POSTGRES_ADMIN_USER`, `POSTGRES_ADMIN_PASSWORD`, `POSTGRES_HOST`, `POSTGRES_PORT`, and per-service `FLEET_SERVICE_DB_USER`/`_PASSWORD`/`_NAME` and `DELIVERY_SERVICE_DB_USER`/`_PASSWORD`/`_NAME`), substituted into `docker-compose.yml` via `${VAR}` interpolation. `POSTGRES_HOST`/`POSTGRES_PORT` are shared (both services talk to the same container, same idea as `KAFKA_BOOTSTRAP_SERVERS`) — only the per-service triple actually varies. Each app service's own `environment:` block re-exposes these under the generic `POSTGRES_HOST`/`POSTGRES_PORT`/`POSTGRES_DB`/`POSTGRES_USER`/`POSTGRES_PASSWORD` names (aliased from its own `FLEET_SERVICE_*`/`DELIVERY_SERVICE_*` vars) — that's the shape `db_client.py` and each service's `migrations/env.py` read from `os.environ` to build their connection URL, with `POSTGRES_HOST` defaulting to `localhost` (not `postgres`) when unset, mirroring `kafka_client.py`'s `KAFKA_BOOTSTRAP_SERVERS` default so commands run from a local `.venv` (like `alembic revision`) work without Docker. To make that actually reachable, the `postgres` service also publishes `5432:5432` to the host, the same rationale as Kafka's `EXTERNAL` listener on `localhost:9094`. Compose only auto-loads a project-level env file named exactly `.env` in the project directory — a differently-named file (e.g. `local.env`) is silently ignored unless every invocation passes `--env-file`, which would break the plain `docker compose up` documented above, so the file must keep that name and live at the repo root. `POSTGRES_USER`/`POSTGRES_PASSWORD` on the `postgres` service itself are read by the official Postgres entrypoint to create the container's own admin superuser (`fleetpulse_admin`) on first boot; this superuser is bootstrap-only and is not what the app services connect with at runtime.
...
**Postgres 15+ schema-privilege gotcha:** `create_db_and_user` grants both `GRANT ALL PRIVILEGES ON DATABASE ... TO ...` and, separately, `GRANT ALL ON SCHEMA public TO ...` (both scoped to that service's own database, since `public` is a distinct per-database schema). The second grant is required because Postgres 15 revoked `CREATE` on the `public` schema from the implicit `PUBLIC` role every user previously inherited it from — the database-level grant alone lets a service user connect and query, but not `CREATE TABLE`, so a service's first Alembic migration fails with `permission denied for schema public` without it.
...
- **`.env` loading and aliasing:** `env.py` reads `.env` via `python-dotenv`'s `dotenv_values()` — not `load_dotenv()`, and not straight into `os.environ`. `load_dotenv()` would also import `.env`'s `POSTGRES_HOST=postgres` (the docker-network hostname) into the process, defeating `db_client.py`'s `POSTGRES_HOST` default of `localhost` meant for exactly this local-run case (a variable that's actually set is no longer eligible for the "unset → default" fallback). Only the per-service `*_DB_USER`/`*_DB_PASSWORD`/`*_DB_NAME` values are pulled from the loaded dict and aliased onto the generic `POSTGRES_USER`/`PASSWORD`/`DB` names — the same aliasing `docker-compose.yml`'s `environment:` block does for containers, done in Python for the local-without-Docker case. This is necessarily duplicated by hand per service (`FLEET_SERVICE_DB_*` vs `DELIVERY_SERVICE_DB_*` in each service's own `env.py`), same as everywhere else in this project.
- **`str(url)` masks the password:** SQLAlchemy's `URL.__str__` always renders the password as `***` (a 1.4+ safety default). Passing `str(sqlalchemy_url)` to `config.set_main_option('sqlalchemy.url', ...)` silently authenticates with the literal string `***`. Use `sqlalchemy_url.render_as_string(hide_password=False)` instead.
- **`ConfigParser` and `%`:** Alembic's `Config` stores `sqlalchemy.url` in a `ConfigParser`, which treats `%` as its own interpolation marker — a percent-encoded password (e.g. `%5E` for `^`) then raises `ValueError: invalid interpolation syntax` when read back out. Rather than escaping every `%` as `%%`, `run_async_migrations()` sidesteps this (and the masking gotcha above) entirely by building the engine directly from the `URL` object — `create_async_engine(_build_database_url(), poolclass=pool.NullPool)` — never round-tripping the URL through `config`/`ConfigParser` as a string. `config.set_main_option` is left unused for the online path; only `run_migrations_offline()` would need it, and that raises `NotImplementedError` here (unused so far).
- **`target_metadata` needs the ORM model imported, not just `Base`:** a table only registers on `Base.metadata` once the module defining it actually runs. `env.py` imports `Base` from `db_client` *and* the service's ORM model module (e.g. `import app.models.orm.delivery`) before capturing `target_metadata = Base.metadata` — skipping the model import silently produces an empty `--autogenerate` diff (no error, just an empty `upgrade()`/`downgrade()`).
- **A literal `$` in a `.env` password breaks Docker Compose, not Python:** Compose's variable interpolation treats `$X` in a `.env` value as a reference to another variable named `X` and silently substitutes blank if `X` is undefined (logged as a `"The '<x>' variable is not set"` warning) — a generated password containing e.g. `$b` gets silently truncated in every container that receives it, while `dotenv_values()` reads the same `.env` literally, with no interpolation. The two disagree on what the password actually is, surfacing as `InvalidPasswordError` only in locally-run tools that go through `dotenv_values()`, not in the containers (all consistently mangled the same way). Generated passwords should avoid `$` for this reason; if one already contains it, escaping as `$$` preserves the intended value through Compose's interpolation.
- **Revision IDs stay Alembic's default random hash**, not hand-rolled sequential numbers (`--rev-id 001`, `002`, ...) — considered, but skipped since each service's `migrations/versions/` and `alembic_version` table are already fully independent (separate databases), so there's no cross-service ordering a sequential scheme would help with.
...
- Add real persistence for Fleet/Delivery Services (currently in-memory only, lost on restart). Prerequisite for the truck position tracking feature below too, since `tracking_service` will need a real repository. Design decisions already made:
  ...
  2. ✅ Done — `sqlalchemy[asyncio]` + `asyncpg` + `alembic` added to each service's `pyproject.toml` and installed into each local `.venv` (the usual uv-workspace install gotcha applied — see Test Conventions). `app/clients/db_client.py` added per service, mirroring `kafka_client.py`'s shape: module-level `AsyncEngine`/`async_sessionmaker` singletons behind `start_db()`/`stop_db()` (wired into each service's `lifespan`, alongside the existing Kafka start/stop calls), a `get_session()` async-context-manager for the "one session per repository call" pattern, and the shared `Base` (`DeclarativeBase`) both services' ORM models inherit from. Each service ran `alembic init -t async migrations` (the async template — the plain sync template's `env.py` doesn't work with an `asyncpg`-only setup) and its `env.py` reads the same five `POSTGRES_*` env vars as `db_client.py` to build its migration connection URL via `sqlalchemy.engine.URL.create(...)`, set with `config.set_main_option("sqlalchemy.url", ...)` before `context.configure` runs — `alembic.ini`'s own `sqlalchemy.url` stays a placeholder, same reasoning as never hardcoding `KAFKA_BOOTSTRAP_SERVERS`.
  3. ✅ Done — SQLAlchemy ORM table models (`app/models/orm/truck.py`'s `Truck`, `app/models/orm/delivery.py`'s `Delivery`), kept separate from the existing Pydantic domain models — Pydantic stays the API/service-layer boundary, the ORM models are repository-internal only. Notable decisions: `TruckStatus`/`DeliveryStatus`/`DeliveryDenialReason` (the existing Pydantic enums) are reused directly as SQLAlchemy column types via `Mapped[SomeEnum]` — SQLAlchemy 2.0 auto-maps a Python `Enum` annotation to a native DB enum type with no extra config, and reusing the enum class itself is fine (a closed vocabulary isn't an API-shape decision, so it doesn't reopen the Pydantic/ORM duplication concern). `Delivery.assigned_truck_id` stays a plain `Mapped[str | None]` column, never a `relationship()`/`ForeignKey` to `Truck` — `fleet_service` and `delivery_service` are separate logical Postgres databases, and Postgres FKs can't cross databases, so this isn't just a service-boundary preference but a hard constraint. `Truck.status` stays a stored column (not derived) for now — deriving it from an assignment-history table instead is a real alternative (see the `Assignment` history idea under Later), but stored is simpler and matches how `assignment_service.py` already directly mutates and re-saves it.
  4. ✅ Done — `truck_repository`/`assignment_repository` (Fleet Service) and `delivery_repository` (Delivery Service) rewritten against Postgres, signatures unchanged (now `async def`, with an `await` added at each call site in the services/consumers). Each repository has its own explicit `_to_orm`/`_from_orm` mapping pair, field by field — no Pydantic `from_attributes`/`model_validate` shortcut. `db_client.get_session()` wraps `get_session_factory().begin()` (not a plain session), so every call gets commit-on-success/rollback-on-exception for free with no explicit `commit()` needed in the repositories themselves. `assignment_repository` needed no real changes, as expected — it only ever queried `truck_repository` live.
  5. ✅ Done — first Alembic migration for both services (`deliveries`, `trucks` tables), generated and applied. See "Alembic migrations" under Architecture for the gotchas hit along the way (password masking/encoding, Postgres 15 schema privileges, `target_metadata` registration, `.env` `$`-interpolation). One loose end: `trucks`' applied migration predates the `plate_number` `unique=True` addition to the ORM model (added after a bug was found in `get_truck_by_plate_number` — it was doing a primary-key lookup via `session.get()` using `plate_number` instead of `id`, so duplicate-plate validation always silently passed) — the live table doesn't enforce that uniqueness yet, needs one more `--autogenerate` + `upgrade head`.
  6. Repository tests currently test pure in-memory dict logic; both repositories now hit Postgres for real, so this decision is live. Still open: testcontainers Postgres (consistent with how the `kafka` + `integration` tests work) vs. something lighter.
