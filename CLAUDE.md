...
Truck assignment used to go through a synchronous HTTP call — `delivery_service/app/clients/fleet_client.py` (`httpx`, async) calling Fleet Service's `POST /internal/truck-assignments`, base URL from the `FLEET_SERVICE_URL` env var. That module (and the `except TimeoutException` handling in `delivery_routes.py` that went with it) has been removed now that `create_delivery` produces to Kafka instead (see Architecture > Kafka below) — the internal HTTP route itself (`POST /internal/truck-assignments`) is still there on the Fleet Service side, kept temporarily for manual testing (see "Fate of `/internal/truck-assignments`" below). The `FLEET_SERVICE_URL` env var itself has since been removed from `delivery_service`'s `docker-compose.yml` entry too, as the last remaining dead trace of that path — nothing in the repo reads it anymore.

### Kafka
A single-node broker (`confluentinc/cp-kafka`, KRaft mode — no Zookeeper) runs as the `kafka` service in `docker-compose.yml`, with a combined `broker,controller` role. It exposes three listeners: `CLIENT` (`kafka:9092`, for other containers — used by `fleet_service`, `delivery_service`, `kafka_init`, `redpanda_console`), `EXTERNAL` (published to the host as `localhost:9094`, so non-containerized local runs, e.g. plain `uvicorn`, can reach the broker too — Kafka's advertised-listener metadata means a single listener can't correctly serve both container and host clients), and `CONTROLLER` for KRaft's internal Raft consensus. The `kafka:9092` value itself is defined once in `docker-compose.yml` via a top-level YAML anchor (`x-kafka-bootstrap: &kafka-bootstrap kafka:9092`) and referenced with `*kafka-bootstrap` from `fleet_service`'s and `delivery_service`'s `KAFKA_BOOTSTRAP_SERVERS`, `kafka_init`'s `KAFKA_BOOTSTRAP_SERVERS`, and `redpanda_console`'s `KAFKA_BROKERS` — a single source of truth in the compose file rather than the literal repeated four times (plain YAML aliasing, not a Compose-specific feature; distinct from `.env`-file `${VAR}` substitution, which is for values that should vary per environment rather than a fixed in-network hostname like this one).

Topics are created explicitly and automatically on startup by a one-shot `kafka_init` service, which runs `infra/kafka/create_topics.sh` (a POSIX `sh` script, idempotent via `--if-not-exists`) once `kafka` reports healthy, then exits. `kafka_init` now gets `KAFKA_BOOTSTRAP_SERVERS` from `docker-compose.yml` (via the anchor above) and the script reads it with a fallback default — `KAFKA_URL="${KAFKA_BOOTSTRAP_SERVERS:-kafka:9092}"` — rather than the value being hardcoded in the script itself. Both app services `depends_on` both `kafka` (`condition: service_healthy`) and `kafka_init` (`condition: service_completed_successfully`), so they don't start until the broker is up and topics exist.
...
**Producer delivery confirmation:** `aiokafka`'s `AIOKafkaProducer.send()` only awaits topic-metadata resolution and placing the message into the local batch accumulator; it returns an `asyncio.Future` that resolves separately once the broker actually acknowledges the write (that's what `send_and_wait` awaits on top: `future = await send(...); return await future`). Both `assignment_producer.py` modules (Fleet Service and Delivery Service) used to do `await get_producer().send(...)` and discard that returned future — so a broker-level failure *after* the message is queued (timeout, leader unavailable, etc.) was never observed by our code; `aiokafka` just logged "exception was never retrieved." For Delivery Service specifically, this meant a `create_delivery` call could return `201`/`REQUESTED` while the actual produce silently failed, leaving the delivery stuck in `REQUESTED` forever with no error anywhere. Fixed by staying fire-and-forget (deliberately not `send_and_wait`, to avoid adding a broker round-trip to the request path) but no longer discarding the future: both modules now capture it and attach `future.add_done_callback(functools.partial(_log_send_failure, topic, delivery_id))`, where `_log_send_failure` calls `future.exception()` (not `.result()`, which would just re-raise inside the callback instead of being observable) and, if not `None`, logs it via `logger.error(..., exc_info=exc)` (not `logger.exception`, since that reads from `sys.exc_info()` which is only populated inside an active `except` block, not in a done-callback holding the exception object directly). This is visibility only — the message is still lost on a genuine broker-level failure, no retry or dead-letter mechanism exists yet — but the failure is now logged instead of silently vanishing. The corresponding call-shape unit tests (`test_assignment_producer.py`, both services) were also updated: `mock_producer.send.return_value` is now explicitly set to a plain `MagicMock()` rather than left as the default `AsyncMock`-chained return value, since `future.add_done_callback(...)` otherwise created an unawaited-coroutine `RuntimeWarning` (the mock's default child attributes are `AsyncMock` all the way down, so `.add_done_callback` was being treated as awaitable when the real `asyncio.Future.add_done_callback` is a plain sync method).
...
- Producer delivery confirmation gap fixed in both `assignment_producer.py` modules (Fleet Service and Delivery Service): the previously-discarded `send()` future is now captured and given a `done_callback` that logs (with traceback) if the broker ultimately fails to ack — fire-and-forget by design (no `send_and_wait`, to avoid adding broker round-trip latency to the request path), visibility only for now (no retry/dead-letter). Corresponding unit tests updated to avoid an `AsyncMock`-chaining `RuntimeWarning` this surfaced. See "Producer delivery confirmation" under Architecture > Kafka for full detail.
- Kafka URL externalized: `infra/kafka/create_topics.sh` no longer hardcodes `kafka:9092`, it reads `KAFKA_BOOTSTRAP_SERVERS` (falling back to `kafka:9092` if unset) — matching the env var name the app services already use for the same value. `docker-compose.yml` now defines that value once via a top-level YAML anchor and references it from `fleet_service`, `delivery_service`, `kafka_init`, and `redpanda_console` instead of repeating the literal — see Architecture > Kafka for the mechanism. Along the way, the fully-dead `FLEET_SERVICE_URL` env var (last remnant of the removed synchronous `fleet_client` HTTP path — nothing in the repo read it anymore) was also removed from `delivery_service`'s `docker-compose.yml` entry.
