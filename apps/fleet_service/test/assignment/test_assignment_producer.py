...
class TestTruckAssignmentRequestedProducer:
    class TestProduceTruckAssignmentCompleted:
        ...
        def test_registers_log_send_failure_as_the_futures_done_callback(self, monkeypatch):
            ...
            assert callback.func is assignment_producer._log_send_failure
            assert callback.args == ("truck-assignment-completed", "delivery-abc123")
    ...
    class TestLogSendFailure:
        def test_logs_error_when_future_resolved_with_exception(self, caplog):
            ...
            with caplog.at_level(logging.ERROR):
                assignment_producer._log_send_failure("truck-assignment-completed", "delivery-abc123", future)

            # - Assert Result -
            assert len(caplog.records) == 1
            record = caplog.records[0]
            assert record.levelno == logging.ERROR
            assert "truck-assignment-completed" in record.getMessage()
